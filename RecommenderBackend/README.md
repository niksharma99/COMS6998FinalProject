...
RecommenderBackend/
│
├── recommender.py              # Core conversational recommender pipeline
├── embedding_loader.py         # Loads embedding artifacts into memory
├── vector_index.py             # FAISS inner-product search index
├── gpt_reranker.py             # Optional: GPT-based reranking module
├── llm.py                      # Lightweight wrapper for OpenAI API calls
├── user_store.py               # Persistent taste-vector memory
│
├── letterboxd_collect_dataset.py     # Scrape/prep Letterboxd-style data
├── letterboxd_to_finetune.py         # Convert scraped data → GPT fine-tuning JSONL
├── finetune_movie_pref.py            # Launch GPT-4o-mini fine-tune
│
├── eval_embedding_alignment.py       # Quantitative retrieval evaluation
├── eval_qualitative_gpt.py           # GPT-as-a-judge qualitative eval
├── qualitative_eval.py               # Human-readable qualitative samples
│
└── README.md                  # (this file)
```
...
* free-text preferences (e.g., *“I like slow atmospheric sci-fi”*),
...
* (\mathbf{e}_t) is the newest embedding from the user’s message
...
To refine the Top-20 retrieved items, we apply an LLM-based reranker using a **custom fine-tuned GPT-4o-mini model**, trained to predict how much a user would rate a movie (1–5).
...
# 📚 Training Details (GPT-4o-mini Fine-Tuning)
...
* `letterboxd_collect_dataset.py`
  → collects user rating & review data (Letterboxd-style)

* `letterboxd_to_finetune.py`
  → converts each example into training pairs:

```
User Profile + Candidate Movie → Expected Rating (1–5)
```
...
# 📊 Evaluation Overview (High-Level)
